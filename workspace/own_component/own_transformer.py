...
'''
##############################################################################
############# define my own designed block-wise transformer here #############
############################################################################## 
'''
...
import own_common_attention as common_attention
import own_transformer_layer as own_transformer_layer
...
transformer_prepare_encoder = own_transformer_layer.transformer_prepare_encoder
transformer_encoder = own_transformer_layer.transformer_encoder
transformer_ffn_layer = own_transformer_layer.transformer_ffn_layer
...
class own_transformer_ssl(t2t_model.T2TModel):
  """Attention net.  See file docstring."""

  def __init__(self, *args, **kwargs):
    super(own_transformer_ssl, self).__init__(*args, **kwargs)
  ...
  # added by mjc: add an hook to realize ssl finetune
  @staticmethod
  def train_hooks(hook_context):
    return []
    # return []

  # added by mjc: add an evaluation hook to get the model sparisty value
  @staticmethod
  def eval_hooks(hook_context):
    # class _SparsityEvalHook(tf.train.SessionRunHook):
    #   def __init__(self, zero_threshold=0.001):
    ...
    #       dims = train_var.get_shape().as_list()
    #       num_dims = len(dims)
    #       if num_dims == 2:
    ...
    #         self.sparsity[sp_name + '_elt_sparsity'] = s
    #         s = tf.nn.zero_fraction(tf.reduce_sum(tf.square(train_var), axis=0))
    #         self.sparsity[sp_name + '_col_sparsity'] = s
    #         s = tf.nn.zero_fraction(tf.reduce_sum(tf.square(train_var), axis=1))
    #         self.sparsity[sp_name + '_row_sparsity'] = s

    #   def after_create_session(self, session, coord):
    #     self.sparsity_results = session.run(self.sparsity)
    #     tf.logging.info("### transformer.py: weight sparisty: %s", self.sparsity_results)

    #   def before_run(self, run_context):
    #     pass
    #     # tf.logging.info("### transformer.py: _LoggerHook before_run")
    #     # return tf.train.SessionRunArgs(loss)  # Asks for loss value.

    #   def after_run(self, run_context, run_values):
    #     pass
        # tf.logging.info("### transformer.py: _LoggerHook after_run")
    return []
    # return [_SparsityEvalHook()]
...
@registry.register_model
class own_transformer_ssl_Scorer(own_transformer_ssl):
  ...
  def __init__(self, *args, **kwargs):
    super(own_Transformer_ssl_Scorer, self).__init__(*args, **kwargs)
    self._name = "own_transformer_ssl"
    self._base_name = "own_transformer_ssl"
