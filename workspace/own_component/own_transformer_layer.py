...
from tensor2tensor.layers.common_layers import dropout_with_broadcast_dims
...
# added by mjc
def blocked_dense(x, filter_size, use_bias, activation, layer_collection, name, input_output_size_ratio):
  """Identical to layers.dense."""
  assert (layer_collection==None)
  if input_output_size_ratio<1:
    in_group, out_group = 2, int(2/input_output_size_ratio)
  else:
    in_group, out_group = int(2*input_output_size_ratio), 2
  activations = dense_block_wise_decouple(input_tensor=x, out_size=filter_size, in_group=in_group, out_group=out_group, name=name, use_bias=use_bias, split_dim=2, group_dim=0)
  if activation:
    activations = activation(activations, name=name)
  return activations
...
def transformer_ffn_layer(x,
  ...
  if ffn_layer == "dense_relu_dense": #          mjc: this path
    ...
    # changed by mjc
    assert (layer_collection==None)
    conv_output = dense_relu_dense(
        x,
        hparams.filter_size, # filter_size is 2048 in default transformer
        hparams.hidden_size, #  hidden_size is 512 in default transformer
