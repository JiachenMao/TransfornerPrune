...
def compute_attention_component(antecedent,
  ...
  if filter_width == 1:
    ...
    # Because original input dim is 512 and output dim is also 512, which will be divided by num_heads(default: 8 in paper)
    # Therefore, we also divide the in_group by num_heads to make each sub-block become square
    return dense_block_wise_decouple(antecedent, total_depth, in_group=num_heads, out_group=num_heads, name=name, use_bias=False, split_dim=2, group_dim=0)
...
def compute_qkv(query_antecedent,
  ...
  # print ('q_filter_width', str(q_filter_width))
...
def multihead_attention(query_antecedent,
  ...
  Args:
    ...
    total_key_depth: an integer
    total_value_depth: an integer
    output_depth: an integer (hidden_size=512 in default transformer)
    num_heads: an integer dividing total_key_depth and total_value_depth
    dropout_rate: a floating point number
  ...
  with tf.variable_scope(name, default_name="multihead_attention",
    ...
    if cache is None or memory_antecedent is None:
      q, k, v = compute_qkv(query_antecedent, memory_antecedent,
                            total_key_depth, total_value_depth, num_heads, q_filter_width,
    ...
    if cache is not None:
      ...
      if memory_antecedent is not None:
        ...
        q = compute_attention_component(query_antecedent, total_key_depth,
                                        q_filter_width, q_padding, "q", num_heads=num_heads,
                                        vars_3d_num_heads=vars_3d_num_heads)
        k = cache["k_encdec"]
        v = cache["v_encdec"]
      else:
    ...
    if vars_3d:
      ...
      assert (layer_collection==None)
      x = dense_block_wise_decouple(x, output_depth, in_group=2, out_group=2, name='output_transform', use_bias=False, split_dim=2, group_dim=0)
      # x = common_layers.dense(
      #     x, output_depth, use_bias=False, name="output_transform",
      #     layer_collection=layer_collection)
    tf.summary.histogram('attention_output', x)
