...
def create_run_config(hp, output_dir=None):
  ...
  tf.logging.info("### t2t_trainer.py: FLAGS.local_eval_frequency %d", FLAGS.local_eval_frequency)
...
def profile_context():
  if FLAGS.profile:
    tf.logging.info("t2t_trainer.py profile_context()  FLAGS.profile: %s" % FLAGS.profile)
...
def execute_schedule(exp):
  ...
  with profile_context():
    tf.logging.info("t2t_trainer.py execute_schedule start")
    tf.logging.info("t2t_trainer.py execute_schedule(exp)  FLAGS.schedule: %s" % FLAGS.schedule)
    getattr(exp, FLAGS.schedule)()
    tf.logging.info("t2t_trainer.py execute_schedule finish")
...
def main(argv):
  ...
  tf.logging.info("t2t_trainer.py start create_experiment_fn")
  exp_fn = create_experiment_fn()
  
  tf.logging.info("t2t_trainer.py initialize experiment_fn with config and get an T2TExperiment object")
  exp = exp_fn(create_run_config(hparams), hparams)
  
  if is_chief():
    save_metadata(hparams)
  tf.logging.info("t2t_trainer.py start execute_schedule")
  execute_schedule(exp)
  tf.logging.info("t2t_trainer.py finish execute_schedule")
