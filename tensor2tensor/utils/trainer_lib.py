...
def create_estimator(model_name,
  ...
  tf.logging.info("trainer_lib.py finish create_estimator")
  return estimator
...
class T2TExperiment(object):
  ...
  def continuous_train_and_eval(self, continuous_eval_predicate_fn=None):
    tf.logging.info("trainer_lib.py start T2TExperiment.continuous_train_and_eval()")
    del continuous_eval_predicate_fn
    tf.estimator.train_and_evaluate(self._estimator, self._train_spec,
                                    self._eval_spec)
    tf.logging.info("trainer_lib.py finish T2TExperiment.continuous_train_and_eval()")
...
def create_experiment(
  ...
  tf.logging.info("### trainer_lib.py: final train_hooks : %s ", train_hooks)
  ...
  tf.logging.info("### trainer_lib.py: final eval_hooks : %s ", eval_hooks)
