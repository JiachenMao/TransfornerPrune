...
import pickle
'''
define all the hooks for structure hoyer square
(1) SHSEvalHook:
      SHSEvalHook is only used during the SHS training step with Group Lasso regularization.
      SHSEvalHook is executed during evaluation, which zerouts
      weights by small threshold to stablize the sparsity
(2) SHSFinetuneHook:
      SHSFinetuneHook is only used during the SHS finetune step to restore the accuracy.

For a complete SHS training procedure, a pretrained model should first do SHS training, then do SSL fine tuning:
pretrained model -> [SHS training] -> [SHS fine tuning] -> final model
'''
...
def print_sparsity_info(sparsity_results, print_level=0, save_sparsity_info=False):
  ...
  tf.logging.info("##############################################")
  structured_zeros, total_params, layer_param = 0, 0, 0
    ...
    for v in var_list:
      if v.op.name == layer_name:
        ...
        if print_level>0:
          ...
          layer_param = dims[0]*dims[1]
          total_params += layer_param
        break
    if print_level>0:
      tf.logging.info("    Overall Random Sparsity: %0.4f %%", sparsities['elt']*100)
      tf.logging.info("        Row Sparsity: %0.4f %%", sparsities['row']*100)
      structured_zeros += (sparsities['row'] + sparsities['col'] - (sparsities['row']*sparsities['col'])) * layer_param
      tf.logging.info("        Column Sparsity: %0.4f %%", sparsities['col']*100)
  tf.logging.info("Overall structured sparisty: %0.4f %%", (float(structured_zeros)/total_params)*100)
...
class SHSEvalHook(tf.train.SessionRunHook):
      ...
      def after_create_session(self, session, coord):
        ...
        print_sparsity_info(sparsity_results, print_level=1)
...
# class SHSInitialWeightHook(tf.train.SessionRunHook):
#       def __init__(self, warm_start_from):
#         tf.logging.info("shs_hooks.py: Checkpoint dir: %s", warm_start_from)
...
class SHSFinetuneHook(tf.train.SessionRunHook):
      def __init__(self, zero_threshold):
        self.zero_threshold = zero_threshold
      ...
      def begin(self):
        tf.logging.info("### transformer.py: _SHSfinetuneHook begin")
