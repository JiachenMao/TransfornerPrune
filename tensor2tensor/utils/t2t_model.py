...
from tensor2tensor.utils import ssl_hooks
from tensor2tensor.utils import shs_hooks
...
class T2TModel(base.Layer):
  ...
  def get_train_hooks(model_name, hook_context):
    model_cls = registry.model(model_name)
    train_hooks = model_cls.train_hooks(hook_context)
    tf.logging.info("### t2t_model.py: get_train_hooks for model: %s --- %s", model_name, train_hooks)
    return train_hooks
    
  @staticmethod
  def get_eval_hooks(model_name, hook_context):
    model_cls = registry.model(model_name)
    eval_hooks = model_cls.eval_hooks(hook_context)
    tf.logging.info("### t2t_model.py: get_eval_hooks for model: %s --- %s", model_name, eval_hooks)
    return eval_hooks
  ...
  def estimator_model_fn(cls,
    ...
    if mode == tf.estimator.ModeKeys.EVAL:
      tf.logging.info("### t2t_model.py: tf.estimator.ModeKeys.EVAL")
  ...
  def estimator_spec_train(self, loss, num_async_replicas=1, use_tpu=False):
    ...
    if use_tpu:
      ...
      if self._hparams.warm_start_from:
        log_info("@@@@@ warm_start_from %s ." % self._hparams.warm_start_from)
        self.initialize_from_ckpt(self._hparams.warm_start_from)
        log_info("t2t_model.py finish initialize_from_ckpt")

      '''
      added by mjc: whether the training is SSL finetune
      if use ssl finetune, add a training hook
      '''
      training_hooks=[]
      if self._hparams.get('ssl_finetune', default=None) is not None:
        if self._hparams.ssl_finetune:
          # problem = self._hparams.problem
          training_hooks += [ssl_hooks.SSLFinetuneHook(zero_threshold=self._hparams.ssl_zero_threshold)]
          # problem.train_hooks(features, logits, self._hparams)
      if self._hparams.get('ssl_regularization', default=None) is not None:
        if self._hparams.ssl_regularization:
          # problem = self._hparams.problem
          training_hooks += [ssl_hooks.SSLInitialWeightHook(warm_start_from=self._hparams.warm_start_from)]
      ...
      return tf.estimator.EstimatorSpec(
          tf.estimator.ModeKeys.TRAIN, loss=loss, train_op=train_op, training_hooks=training_hooks)
  ...
  def estimator_spec_eval(self, features, logits, labels, loss, losses_dict):
    ...
    if common_layers.is_xla_compiled():
      ...
      tf.logging.info("### t2t_model.py: estimator_spec_eval task_list: %s", task_list)
      ...
      # added by mjc: check whether to print sparsity during evaluation
      # SSLEvalHook is only used during the SSL training step with Group Lasso regularization.
      if self._hparams.get('print_sparsity', default=None) is not None:
        if self._hparams.print_sparsity:
          # evaluation_hooks += ssl_hooks.SSLEvalHook(zero_threshold=1)
          if self._hparams.get('ssl_regularization', default=None) is not None and self._hparams.ssl_regularization:
            evaluation_hooks.append(ssl_hooks.SSLEvalHook(zero_threshold=self._hparams.ssl_zero_threshold))
          if self._hparams.get('shs_regularization', default=None) is not None and self._hparams.shs_regularization:
            evaluation_hooks.append(shs_hooks.SHSEvalHook(zero_threshold=self._hparams.shs_zero_threshold))
          # evaluation_hooks += problem.eval_hooks(features, logits, hparams)
      
      tf.logging.info("### t2t_model.py: evaluation_hooks is: %s", evaluation_hooks)
...
def initialize_from_ckpt(ckpt_dir, hparams):
  ...
  """the following codes are command out because 
  I don't want the code the automatically load checkpoint from model_dir  
  """
  """
  ...
  if already_has_ckpt:
    log_info("mjc_log_t2t_model: already has check point from %s, so not load new check point from user defined dir" % model_dir)
    return
  """

  tf.logging.info("t2t_model.py: Checkpoint dir: %s", ckpt_dir)
