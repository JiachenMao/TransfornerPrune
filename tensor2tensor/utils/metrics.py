...
def structure_sparsity(predictions, labels, weights_fn=None):
  """compute row wise and column wise sparsity of model weight."""
  tf.logging.info("### metrics.py: structure_sparsity")
  ...
  for v in var_list:
    ...
    if not is_bias:
      with tf.device(v.device):
        ...
        num_dims = len(dims)
        if num_dims == 2:
          # tf.logging.info("Layer: %s, is of dimensions: %s", v, dims)
          # tf.logging.info("Layer: %s, sum of weights: %s", v, v[1,1])
          # percentage, _ = tf.metrics.percentage_below(values=v, threshold=0.1)
          # tf.reduce_sum(v)
          percentage_list.append(tf.reduce_sum(v))
          # percentage, _ = tf.metrics.percentage_below(values=v, threshold=1)
          # percentage_list.append(percentage)
  return tf.add_n(percentage_list), tf.constant(2)
...
def create_evaluation_metrics(problems, model_hparams):
  ...
  for problem_instance in problems:
    ...
    tf.logging.info("### metrics.py: create_evaluation_metrics for problem: %s", problem_name)
    if problem_instance.was_reversed:
      problem_name += "_rev"
    tf.logging.info("### metrics.py: problem_instance: %s", problem_instance)
    metrics = problem_instance.eval_metric_fns(model_hparams)
    if hasattr(model_hparams.problem, "task_list"):
      tf.logging.info("### metrics.py: hasattr - : task_list")
      metrics = model_hparams.problem.eval_metric_fns(model_hparams)

    tf.logging.info("### metrics.py: metrics: %s", metrics)
    ...
    for target_name, modality in six.iteritems(tm):
      ...
      for metric, metric_fn in six.iteritems(metrics):
        ...
        if len(problems) == 1 and overload_eval_metric_name:
          ...
          tf.logging.info("### metrics.py: len(problems) == 1 and overload_eval_metric_name metric_name: %s", metric_name)
        else:
          metric_name = "metrics-%s/%s/%s" % (problem_name, target_name, metric)
          tf.logging.info("### metrics.py: metric_name: %s len(problems) is: %d", metric_name, len(problems))
        if metric == Metrics.IMAGE_SUMMARY:
          eval_metrics[metric_name] = make_image_wrapped_metric_fn(metric_fn)
        else:
          tf.logging.info("### metrics.py: make_problem_specific_metric_fn")
...
METRICS_FNS = {
    ...
    Metrics.STRUCTURE_SPARSITY: structure_sparsity,
}
